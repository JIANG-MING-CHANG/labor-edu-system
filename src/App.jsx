...
Play, Square, PieChart, CheckCircle2, XCircle, Volume2, VolumeX, MessageSquare, LogOut, Unlock, Gift, Send
} from 'lucide-react';
...
if (!db || !auth) {
return 系統初始化中，或缺乏後端設定。請確認 Firebase 設定。;
}
      ...
      <form onSubmit={handleStudentLogin} className="space-y-6">
        <div>
          ...
          <input 
            ...
            placeholder="例如：王大明 (斷線重連請輸入相同名稱)"
            value={name}
...
}, [session?.status, session?.isTimerRunning, session?.endTime]);

const selectOption = (choice) => {
if (session?.status !== 'active' || !session?.isTimerRunning || hasSubmitted || isIndividuallyLocked) return;
setMyAnswer(choice);
};
...
try {
  ...
  await setDoc(pRef, {
    ...
    [`q_${currentSession.currentQuestion}_text`]: myTextRef.current || (isAuto ? '時間到，系統自動交卷' : ''),
    updatedAt: Date.now()
  }, { merge: true });
} catch(e) {
...
const displayTime = frozenTime !== null ? frozenTime : timeLeft;
  ...
  {broadcastAlert && (
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-slate-900/80 backdrop-blur-md">
      <div className="bg-white dark:bg-slate-900 rounded-3xl shadow-2xl p-8 max-w-sm w-full animate-[shake_0.5s_ease-in-out] border border-blue-500/30">
        <h3 className="text-2xl font-black mb-4 flex items-center text-blue-600 dark:text-blue-400 border-b pb-3 border-slate-200 dark:border-slate-800">
          <MessageSquare className="mr-3 text-blue-500" size={28}/> 講師動態廣播
        </h3>
        <p className="text-lg mb-8 leading-relaxed text-slate-800 dark:text-slate-200 font-medium">
          {broadcastAlert.text}
        </p>
        <button
          onClick={() => setBroadcastAlert(null)}
          className="w-full py-4 bg-gradient-to-r from-blue-600 to-purple-600 hover:from-blue-500 hover:to-purple-500 text-white rounded-xl font-bold shadow-lg hover:scale-105 transition-all flex items-center justify-center text-lg"
        >
          <CheckCircle2 size={24} className="mr-2"/> 我已收到 (確認)
        </button>
      </div>
    </div>
  )}
  ...
  <div className="flex justify-between items-center mb-6 glass-card p-4 rounded-2xl mt-12 md:mt-0">
    ...
    <div className="flex items-center space-x-4">
      ...
      <div className={`flex items-center font-mono text-2xl font-black px-5 py-2 rounded-xl shadow-inner ${
        ...
        {(!session.isTimerRunning && session.status === 'active') ? (
           <span className="text-lg animate-pulse">等待計時</span>
        ) : (
           displayTime !== null ? `${Math.floor(displayTime / 60).toString().padStart(2, '0')}:${(displayTime % 60).toString().padStart(2, '0')}` : '00:00'
        )}
      </div>
    </div>
  </div>
  ...
  {!showResult && (
    <div className="glass-card p-6 rounded-3xl mb-10 border border-slate-300 dark:border-slate-700 shadow-xl">
      ...
      <textarea 
        ...
        rows="3"
        placeholder={session.isTimerRunning ? "輸入您的觀點或法理依據..." : "等待計時開始後，方能進行作答。"}
        value={myText}
...
const [session, setSession] = useState({ status: 'closed', currentQuestion: 0, isTimerRunning: false });
...
setSession({ status: 'closed', currentQuestion: 0, isTimerRunning: false });
}
});
return () => unsub();
}, [user, db, appId]);
...
setDbError("連線逾時！請至 Firebase 建立「Firestore Database」並修改規則。");
} else {
setDbError("權限不足！請至 Firestore Database 修改規則為 allow read, write: if true;");
}
} finally {
setIsUpdating(false);
}
...
const revealAnswer = () => updateSession({ status: 'revealed', isTimerRunning: false });
...
updateSession({ status: 'active', currentQuestion: nextIdx, isTimerRunning: false, expectedCount });
}
};
...
const msg = session.status === 'closed' 
  ...
  : `確定要直接跳至第 ${idx + 1} 關嗎？\n作答將恢復「等待計時」狀態，需重新點擊開始計時。`;
  
if (window.confirm(msg)) {
  updateSession({ status: 'active', currentQuestion: idx, isTimerRunning: false, expectedCount });
}
...
{.cyber-panel { background: ${isDark ? 'rgba(15, 23, 42, 0.6)' : 'rgba(255, 255, 255, 0.7)'}; backdrop-filter: blur(16px); border: 1px solid ${isDark ? 'rgba(56, 189, 248, 0.1)' : 'rgba(56, 189, 248, 0.4)'}; box-shadow: 0 4px 30px rgba(0, 0, 0, 0.1); } .cyber-border { position: relative; } .cyber-border::after { content: ''; position: absolute; bottom: 0; left: 0; right: 0; height: 1px; background: linear-gradient(90deg, transparent, rgba(56, 189, 248, 0.5), transparent); }}
  ...
  <div className="w-full md:w-80 bg-white/90 dark:bg-[#0B1120]/90 border-r border-blue-200 dark:border-blue-900/30 p-6 flex flex-col h-screen overflow-y-auto backdrop-blur-xl relative z-10 shadow-[5px_0_30px_rgba(0,0,0,0.05)] dark:shadow-[5px_0_30px_rgba(0,0,0,0.5)]">
    ...
    <div className="space-y-5 flex-grow">
      ...
      <div className="cyber-panel p-4 rounded-xl space-y-4">
        <div>
          <label className="block text-sm font-bold text-blue-600 dark:text-blue-400 mb-2 flex items-center"><Users size={16} className="mr-2"/>設定預計人數 (人)</label>
          <input 
            type="number" 
            min="0"
            value={expectedCount}
      ...
      <div className="cyber-panel p-4 rounded-xl">
        <label className="block text-sm font-bold text-blue-600 dark:text-blue-400 mb-2 flex items-center">
          <MessageSquare size={16} className="mr-2"/>全體動態廣播
        </label>
        <textarea
          value={broadcastText}
          onChange={(e) => setBroadcastText(e.target.value)}
          rows="2"
          placeholder="輸入推播訊息，學員須強制點擊確認..."
        ...
        />
        <button onClick={sendBroadcast} disabled={isUpdating || !broadcastText.trim()} className="w-full bg-blue-600 hover:bg-blue-700 text-white font-bold py-2 rounded-lg transition-colors disabled:opacity-50 flex items-center justify-center shadow-md shadow-blue-500/20">
          <Send size={16} className="mr-2"/> 發送廣播對話框
        </button>
      </div>
      ...
      {session.status === 'closed' ? (
        ...
        <>
          {session.status === 'active' && !session.isTimerRunning && (
            <button onClick={startTimer} disabled={isUpdating} className="w-full py-4 rounded-xl bg-gradient-to-r from-blue-500 to-indigo-600 hover:from-blue-400 hover:to-indigo-500 font-bold text-white text-lg flex items-center justify-center transition-all shadow-[0_0_20px_rgba(59,130,246,0.5)] hover:scale-[1.03] animate-pulse disabled:opacity-50">
              <Play className="mr-2" size={20}/> 開始計時 (開放作答)
            </button>
          )}
  ...
  <div className="flex-1 p-6 md:p-10 overflow-y-auto h-screen relative bg-[radial-gradient(ellipse_at_top,_var(--tw-gradient-stops))] from-slate-100 via-white to-slate-50 dark:from-slate-900 dark:via-[#0B1120] dark:to-black">
     ...
     {session.status !== 'closed' && session.status !== 'finished' ? (
            ...
            <div className="cyber-panel rounded-3xl p-6 shadow-2xl flex flex-col justify-center relative overflow-hidden">
               ...
               {session.status === 'active' && (
                 <div className="text-center text-blue-600 dark:text-blue-300 relative z-10">
                   <Clock size={56} className={`mx-auto mb-4 drop-shadow-[0_0_15px_rgba(59,130,246,0.6)] ${session.isTimerRunning ? 'animate-[spin_4s_linear_infinite]' : 'opacity-60'}`} />
                   <h4 className="font-black text-2xl mb-3 tracking-widest text-slate-900 dark:text-white">
                     {session.isTimerRunning ? '作答接收中 (計時中)' : '等待開放計時'}
                   </h4>
                   <p className="text-sm opacity-80 text-blue-700 dark:text-blue-200">
                     {session.isTimerRunning ? '監控數據變化，適當時機可切換至討論模式。' : '請於左側面板點擊「開始計時」按鈕，方能開放作答。'}
                   </p>
                 </div>
               )}
               {session.status === 'discussion' && (
            ...
            <div className="grid md:grid-cols-2 gap-6 relative z-10">
               ...
               <div className="bg-purple-50/50 dark:bg-purple-950/40 p-6 rounded-2xl border border-purple-200 dark:border-purple-500/30 backdrop-blur-md shadow-[inset_0_0_20px_rgba(168,85,247,0.05)] dark:shadow-[inset_0_0_20px_rgba(168,85,247,0.1)]">
                  <div className="font-bold text-lg mb-3 flex items-center text-purple-700 dark:text-purple-300">
                     <span className="text-xl mr-2">⚖️</span> 實務判例 / 裁罰原則
                  </div>
                  <p className="text-slate-700 dark:text-purple-50/90 leading-relaxed text-sm font-medium">
                     {currentQ.rulingText.replace('⚖️ 實務裁罰原則：', '').replace('⚖️ 實務案例：', '').replace('⚖️ 實務判例：', '')}
                  </p>
               </div>
            </div>
         </div>
            ...
            <div className="overflow-x-auto">
              <table className="w-full text-left text-sm text-slate-700 dark:text-slate-300">
                <thead className="bg-slate-100 dark:bg-[#0B1120] text-blue-700 dark:text-blue-300 border-b border-blue-200 dark:border-blue-900/50">
                  <tr>
                    <th className="p-4 font-bold tracking-wider">學員姓名 (識別碼)</th>
       ...
       <div className="h-full flex flex-col items-center justify-center opacity-70">
         <ShieldAlert size={100} className="mb-6 text-blue-500 drop-shadow-[0_0_20px_rgba(59,130,246,0.8)] animate-pulse" />
         <h2 className="text-5xl font-black mb-4 tracking-widest text-transparent bg-clip-text bg-gradient-to-r from-blue-500 to-purple-500">SYSTEM STANDBY</h2>
         <p className="font-mono text-xl tracking-widest text-slate-700 dark:text-blue-300 mb-3">目前系統為待命/關閉狀態</p>
         <p className="text-sm font-medium opacity-70 bg-white/50 dark:bg-black/50 px-6 py-2 rounded-full backdrop-blur-md">
           👉 請於左側中控台設定人數與時間，並點擊「開放登入並開始」
         </p>
       </div>
     )}
  </div>
